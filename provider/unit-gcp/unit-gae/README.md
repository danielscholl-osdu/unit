...
* Data-Lake unit Google Cloud Endpoints on App Engine Flex environment
  * Deploy
    ```sh
    mvn appengine:deploy -pl org.opengroup.osdu.unit:unit -amd
    ```

  * If you wish to deploy the unit service without running tests
    ```sh
    mvn appengine:deploy -pl org.opengroup.osdu.unit:unit -amd -DskipTests
    ```
